...
- Native v2 assigns every visible capture exactly once. Subprocess v1 adapts as `planner_protocol=v1_compat` and cannot claim native readiness. Invalid atomicity gets bounded correction; message-only failure uses the locked rewrite path.
- Fallback: Fast uses the smallest materializable hard component; Balanced uses last valid or deterministic dependency partition plus fast verification; Quality keeps pending with `needs_attention`. No preset bypasses hard dependencies, materialization, or required verification.
...
- Planner circuit: transport failure opens immediately; three validation/safety failures open; cooldowns 30s, 2m, 10m; one half-open probe. Caller cancellation releases the probe without changing health.
