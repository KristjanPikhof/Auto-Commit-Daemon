...
- Tags use `vYYYY-MM-DD`. Verify latest with `git tag --sort=-creatordate | head`; local latest at this refresh was `v2026-05-16`. `v2026-05-10` exists at `4d9dfef`. Do not move published tags unless requested.
...
| Cursor | `sessionStart -> acd-lifecycle.sh start` | `postToolUse`/`afterFileEdit` -> `acd-lifecycle.sh wake` (start+wake) | `stop -> flush`; `sessionEnd -> stop` | User-global `~/.cursor/hooks.json` + `./hooks/acd-lifecycle.sh`; `conversation_id`; `--watch-pid 0`; approve in Settings → Hooks |
...
- Cursor: user-global `~/.cursor/hooks.json` only (not repo `.cursor/hooks.json`); install `templates/cursor/hooks/acd-lifecycle.sh` to `~/.cursor/hooks/acd-lifecycle.sh` (commands relative to `~/.cursor/`). `acd setup cursor --raw > ~/.cursor/hooks.json` replaces the entire file; merge the five lifecycle events manually when non-acd hooks exist. Approve in Settings → Hooks after install.
