...
| Strategy | `ACD_COMMIT_STRATEGY=event|intent`; `ACD_INTENT_WINDOW=10`; `ACD_INTENT_MIN_PENDING=10`; `ACD_INTENT_MAX_PENDING_AGE=5m`; `ACD_INTENT_RECENT_COMMITS=5`; `ACD_INTENT_DEFER_LIMIT=1` (Wave 2 lowered from 2; the planner-validation retry loop in `composed.PlanIntent` plus the `<gitDir>/acd/planner-rejects.jsonl` forensic surface make a single deferral overwhelmingly more likely to be planner churn than legitimate "wait for related work"; set `=2` to restore prior tolerance); `ACD_INTENT_PATH_COALESCE=1` (default ON; `0|false|no|off` disables); intent-stage diff cap is fixed at 16 KiB per offered capture (constant `ai.IntentStageDiffCap`, not env-tunable) so the planner sees enough context to group multi-file changes — the per-event commit-message path still uses the legacy 4 KiB `ai.DiffCap` |
