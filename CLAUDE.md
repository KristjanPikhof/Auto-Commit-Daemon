...
Known pre-existing flakes on main (not regressions): `TestReplay_*` skipped via `intent_batch_wait`, `TestDiagnose_IntentBatchWait*`, `TestDoctor_IntentBatchWait*`, `TestSelfHeal_ParallelCommitterDoesNotBlock`, `TestSelfHeal_ManualPauseAndResume`. Verify on `main` before fixing.
...
- `internal/cli`: Cobra commands; setup/hookhelper/doctor/diagnose/recover/commit-all.
...
- `internal/state`: schema v7 (`decision_records` v5; `event_seq` v6; `planner_state` v7); shadow/meta/clients/flush/safe-ignore/sensitive matchers.
...
- `internal/adapter`: harness detection (per-path marker matching). No TODO stubs.
- `internal/{central,identity,logger,paths,pause,trace}`: stats/fingerprints/logs/XDG/pause/trace.
- `templates/{claude-code,codex,opencode,pi,shell}/`: harness snippets. Codex template is `templates/codex/hooks.json` (legacy `config.snippet.toml` removed). Keep `templates/embed.go` current.
...
- HEAD-transition tests wait for `waitForMetaValue(MetaKeyBranchHead, <sha>, 3s)`.
- CLI changes need Cobra help/examples/root-help updates. Template changes must preserve embedded FS.
- Changelog/release notes describe user impact, not file diffs.
- Self-hosting hazard: ACD's own auto-commit daemon runs in this repo. `git checkout main -- .` mid-edit causes the daemon to capture and commit reverts. `acd pause --repo . --reason "..." --yes` before destructive git surgery; `acd resume --yes` after.
...
- Safe-ignore dirs prune descendants, not same-named files. `SafeIgnoreMatcher.MatchFile` for files/symlinks; `MatchDirectory` for dirs.
...
- Deferred captures stay pending in `planner_state` (`defer_count`, `last_planned_ts`, reason/error). At `defer_count >= ACD_INTENT_DEFER_LIMIT`, oldest overdue capture forced one-item.
...
- `blocked_conflict` and `failed` are terminal seq barriers; `PendingEvents` hides later pending behind prior terminal rows for same branch/generation.
...
- Branch transitions set `branchTransitionSettleDelay = 100ms` so ref moves/worktree updates not sampled as separate edits.
...
- `events`/`explain`/`doctor` read paths must not call `state.Open` or migrate DBs. Use read-only SQLite projections (`openStateDBReadOnly`).
...
- `acd setup <harness> --raw` emits snippet body only (no `// `-wrapped header/footer/README). Required for strict-JSON targets like `~/.codex/hooks.json`. Keep `commentPrefix` wrappers as default for paste-into-existing-config flow.
...
- Wired Codex events: `SessionStart` → `acd start` (15s). `UserPromptSubmit` → idempotent `acd start` then `acd wake` (15s). `PreToolUse`/`PostToolUse` → idempotent `acd start` then `acd wake` (matcher `apply_patch|Edit|Write|Bash`, 15s). `Stop` → `acd touch` (5s; mirrors claude-code so replay drain finishes before refcount sweep cleans up).
- Claude Code/OpenCode/Pi active hooks also run idempotent `acd start` before `acd wake`. End-session hooks differ: `SessionEnd` / `session.deleted` → `acd stop --session-id`.
...
- Templates use `acd hook-stdin-extract <field> [field...]` (multi-arg, optional `?`-suffix) instead of `jq`. Keep `internal/cli/hookhelper.go` and AdapterE2E coverage.
