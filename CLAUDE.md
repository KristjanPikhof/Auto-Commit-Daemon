...
- Planner rejects log: `<gitDir>/acd/planner-rejects.jsonl` captures one JSON line per `ValidateIntentPlan` failure — `ts` (RFC3339Nano), `provider`, `offered_seqs`, `raw_response` (verbatim model output), `code` (typed `IntentPlanValidationCode` integer), `message`. Rotates at 5 MiB to `planner-rejects.jsonl.1` (one archive kept). Best-effort writes via `ai.LogRejectedIntentPlan`; failures emit `slog.Warn` and never block replay. `ai.ConfigureIntentRejectsLogger(opts.GitDir)` installs the package-level writer at daemon startup; tests inject via `ai.SetIntentRejectsLoggerForTest`.
...
| Strategy | `ACD_COMMIT_STRATEGY=event|intent`; `ACD_INTENT_WINDOW=10`; `ACD_INTENT_MIN_PENDING=10`; `ACD_INTENT_MAX_PENDING_AGE=5m`; `ACD_INTENT_RECENT_COMMITS=5`; `ACD_INTENT_DEFER_LIMIT=1` (Wave 2 lowered from 2; the planner-validation retry loop in `composed.PlanIntent` plus the `<gitDir>/acd/planner-rejects.jsonl` forensic surface make a single deferral overwhelmingly more likely to be planner churn than legitimate "wait for related work"; set `=2` to restore prior tolerance); `ACD_INTENT_PATH_COALESCE=1` (default ON; `0|false|no|off` disables); `ACD_RECENT_COMMIT_AFFINITY_SECONDS=120` (0 disables; surfaces `path_recent_commits[i]={path,oid,age_seconds,suggested_action="extend or wait"}` hint into the planner user_message JSON when an offered path matches a HEAD commit landed within the window — informational only, no amend in v1, follow-up planned to act on the hint); intent-stage diff cap is fixed at 16 KiB per offered capture (constant `ai.IntentStageDiffCap`, not env-tunable) so the planner sees enough context to group multi-file changes — the per-event commit-message path still uses the legacy 4 KiB `ai.DiffCap` |
