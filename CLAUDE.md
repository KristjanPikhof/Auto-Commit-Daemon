...
- Intent defaults: `ACD_INTENT_WINDOW=10`, `ACD_INTENT_MIN_PENDING=10`, `ACD_INTENT_MAX_PENDING_AGE=5m`, `ACD_INTENT_RECENT_COMMITS=5`, `ACD_INTENT_DEFER_LIMIT=1` (Wave 2 lowered from 2; pairs with the planner-validation retry loop and the rejects-log forensic surface — set `=2` to restore prior tolerance). Per-stage planner diff cap is the constant `ai.IntentStageDiffCap = 16000` (per offered capture); the per-event commit-message path keeps `ai.DiffCap = 4000`. `acd diagnose --json` surfaces the stage cap as `intent_strategy.intent_stage_diff_cap`.
...
- `deferred_reasons[i].seq` MUST appear in `deferred_seqs`; reasons attached to selected/non-offered seqs are invalid. `internal/ai/intent_planner.go` `ValidateIntentPlan` returns typed `*IntentPlanValidationError{Code: IntentPlanValidationDeferredReasonNotDeferred}` for this case. `openai-compat` and `subprocess` providers call `NormalizeIntentPlanDeferredReasons` before validate, drop spurious entries, and emit one `slog.Warn` naming the dropped seqs. `composed.PlanIntent` runs the same normalize+warn on both primary and fallback paths so third-party planners wired through `Compose` are covered; `planIntentWithFallback` runs the normalizer one more time without warn as defense in depth. Plans that still fail validation after normalization fall back through `Compose` as before.
- Deferred stays pending in `planner_state`; at `defer_count >= ACD_INTENT_DEFER_LIMIT`, oldest overdue is forced one-item.
- Same-path coalesce (`internal/daemon/coalesce.go`) folds runs of consecutive captures touching exactly one shared path into one offered planner entry. Boundaries: different path / multi-path / rename / delete event, or any change in `(branch_ref, branch_generation, base_head)`. Squashed entry keeps the run's first `BeforeOID`/`BeforeMode` and the run's last `AfterOID`/`AfterMode`. On commit success every original seq is marked published with the same `commit_oid` and gets its own decision row, so `acd events --json grouped_seqs` covers the whole run. `ACD_INTENT_PATH_COALESCE` defaults ON; `0|false|no|off` disables (restart to apply).
...
- Planner rejects log: `<gitDir>/acd/planner-rejects.jsonl` captures one JSON line per `ValidateIntentPlan` failure — `ts` (RFC3339Nano), `provider`, `offered_seqs`, `code` (typed `IntentPlanValidationCode` integer), `message`, raw response size, sha256, and parsed-plan summary. `raw_response` is redacted by default and is stored verbatim only when `ACD_INTENT_REJECTS_RAW` is truthy; the daemon emits a startup warning in verbatim mode. Rotates at 5 MiB to `planner-rejects.jsonl.1` (one archive kept). Best-effort writes via `ai.LogRejectedIntentPlan`; failures emit `slog.Warn` and never block replay. `ai.ConfigureIntentRejectsLogger(opts.GitDir)` installs the package-level writer at daemon startup; tests inject via `ai.SetIntentRejectsLoggerForTest`.
- `acd status --json` and `acd diagnose --json` `intent_strategy` block surface `planner_error_rate_recent` and `singleton_commit_rate_recent` over the last 100 decisions / 100 distinct commit OIDs respectively. Denominator is fixed at 100 even when the ledger holds fewer rows (rate dilutes toward zero while the ledger fills). `acd diagnose` adds a remediation hint when `planner_error_rate_recent > 0.05`.
...
|---|---|---|---|---|
| Claude Code | `SessionStart -> acd start` fail-soft | `Pre/PostToolUse`: and-chain + log fallback | `Stop -> acd flush --logical`; `SessionEnd -> acd stop --session-id` | `CLAUDE_PROJECT_DIR:-$PWD`; nested JSON |
| Codex | `SessionStart -> acd start` | `UserPromptSubmit`/`PreToolUse`/`PostToolUse`; matcher `apply_patch\|Edit\|Write\|Bash` on `PreToolUse` and `PostToolUse`; mkdir gated | `Stop -> acd touch` (no logical-flush rewire — see note below) | `templates/codex/hooks.json`; active timeout 15s |
| OpenCode | `session.created -> acd start` | `tool.before.*`/`tool.after.*`: and-chain + log fallback | `session.idle -> acd flush --logical`; `session.deleted -> acd stop --session-id` | `OPENCODE_SESSION_ID`/`OPENCODE_PROJECT_DIR`; `~/.config/opencode/hook/hooks.yaml` |
| Pi | `session.created -> acd start` | `tool.before.*`/`tool.after.*`: and-chain + log fallback | `session.idle -> acd flush --logical`; `session.deleted -> acd stop --session-id` | `SID="${PI_SESSION_ID:-pi-$$-$(date +%s)}"`; no `uuidgen`; `~/.pi/agent/hook/hooks.yaml` |
...
- Codex hooks are enabled by default; if `~/.codex/config.toml` pins feature flags, keep them enabled with `[features].hooks = true`. `features.codex_hooks` is only a deprecated alias. `hooks.json` carries hook bodies, not feature flags.
...
- `acd flush --logical` is the explicit drain entrypoint installed at Stop / idle hooks. It refreshes the heartbeat, enqueues a `flush_logical` flush_request, and signals SIGUSR1 to the daemon. Only drained `flush_logical` requests set `IntentBypassBatchWait=true`, which makes the next replay tick evaluate the visible window without waiting for `ACD_INTENT_MIN_PENDING` or `ACD_INTENT_MAX_PENDING_AGE`. Plain `acd wake` requests are acknowledged and nudge capture/replay, but they do not bypass the intent batch gate. Refusals (detached HEAD, in-progress git op, manual pause marker) keep the heartbeat refresh and report `refused_reason` in JSON output but never block the harness hook.
...
| Capture | `ACD_SENSITIVE_GLOBS`; `ACD_SAFE_IGNORE`; `ACD_SAFE_IGNORE_EXTRA`; `ACD_MAX_PENDING_EVENTS`; `ACD_PATH_QUIESCENCE_SECONDS=0` (off; integer seconds; non-zero defers offering pending captures for path P to the planner until P has been quiet for that long — the capture row itself is durable, only the planner-offer is gated; restart to apply; `daemon_meta.path_quiescence.gated_count` snapshot drives status' `path_quiescence_gated_events` field and subtracts from `visible_pending_events` while `oldest_pending_age` stays anchored to `captured_ts`) |
...
| Strategy | `ACD_COMMIT_STRATEGY=event|intent`; `ACD_INTENT_WINDOW=10`; `ACD_INTENT_MIN_PENDING=10`; `ACD_INTENT_MAX_PENDING_AGE=5m`; `ACD_INTENT_RECENT_COMMITS=5`; `ACD_INTENT_DEFER_LIMIT=1` (Wave 2 lowered from 2; the planner-validation retry loop in `composed.PlanIntent` plus the `<gitDir>/acd/planner-rejects.jsonl` forensic surface make a single deferral overwhelmingly more likely to be planner churn than legitimate "wait for related work"; set `=2` to restore prior tolerance); `ACD_INTENT_PATH_COALESCE=1` (default ON; `0|false|no|off` disables); `ACD_RECENT_COMMIT_AFFINITY_SECONDS=0` (off; when `>0`, surfaces `path_recent_commits[i]={path,oid,age_seconds,suggested_action="extend or wait"}` hint into the planner user_message JSON when an offered path matches a HEAD commit landed within the window — informational only, no amend in v1); intent-stage diff cap is fixed at 16 KiB per offered capture (constant `ai.IntentStageDiffCap`, not env-tunable) so the planner sees enough context to group multi-file changes — the per-event commit-message path still uses the legacy 4 KiB `ai.DiffCap` |
