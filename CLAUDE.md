...
- Tags are `vYYYY-MM-DD`. As of 2026-05-10, `v2026-05-10` is published at `4d9dfef`; `main` may be newer due release-workflow fixes. Do not move a published tag unless explicitly requested.
...
- `.github/workflows/release.yml` runs GoReleaser `--skip=homebrew`, then retries install smoke 6 times for GitHub asset propagation. If smoke fails with asset 404 after publish, verify release view, checksums/downloads, and local `ACD_VERSION=<tag> sh scripts/install.sh` before retagging. Reruns can fail because the release already exists.
- `.goreleaser.yaml` sets `prerelease: false`; date tags otherwise become pre-releases and `releases/latest` breaks. Brew remains skipped until tap credentials exist.
...
| `templates/{claude-code,codex,opencode,pi,shell}` | harness snippets; Codex uses `templates/codex/hooks.json`; legacy TOML deleted |
...
- Docs are release contract. Update README/changelog/docs only when stale; changelog/release notes describe user impact, not file diffs.
...
- Races, panics, nil pointers, ordering failures, CI flakes, and read-only path migrations are bugs. Inspect/narrow before retry.
...
- Intent-env failures: rerun with `cleanenv` before declaring a product bug. Verify suspected main flakes on `main`.

## State, Branch, Capture

- Repo DB `<gitDir>/acd/state.db`; central registry/stats use XDG state/share.
- Start short-circuit cache: per-session `<gitDir>/acd/start-cache-<sha256(session_id)[:16]>.json`, schema v2. Read lock-free at top of `runStart`; written after cold-path success; `acd stop` removes matching/all caches. Atomic tmp+rename prevents corrupt concurrent writes.
...
- Shadow bootstrap: 5000-row chunks, marker `shadow.bootstrapped:<branch_ref>:<generation>` only after all chunks commit, partial rows cleaned on failure.
...
- Diverged drops stale `pending` for prior generation and keeps `published`. Dead prior refs trigger `state.PurgeUnpublishedForDeadBranch`: prune `pending` + terminal `blocked_conflict`/`failed`, lift `publish_state` only for deleted blocked row in that pair, clear replay breadcrumbs. Live refs preserve rows/blockers.
- Dead-branch startup sweep runs after running-mode publish on a one-shot goroutine, uses `git.LiveBranchSet`, honors manual pause and `ACD_KEEP_DEAD_BRANCH_BARRIERS=1`, stamps `dead_branch_prune.{last_run_ts,last_count,last_refs}`, and surfaces in `acd diagnose --json`. SQLite read errors in `daemonPauseState` fail closed.
...
- `IgnoreChecker.Check`: long-lived `git check-ignore --stdin -z --non-matching --verbose`; writer goroutine streams stdin while stdout is read. One large `stdin.Write` deadlocks on macOS 16 KiB pipes. `Close` is non-blocking cancel + `killLocked` + bounded `cmd.Wait` 2s.
...
- Hook log: `${XDG_STATE_HOME:-$HOME/.local/state}/acd/<harness>-hook.log`; active hooks append fail-soft diagnostics. Doctor tails Codex logs.
- `acd logs --follow` streams from EOF reached by initial tail read; do not re-`Stat`.
...
- `acd doctor` detects drift when active hooks lack both `acd start` and `acd wake`, falls back to alternate-path detection on EACCES/EIO, tails Codex hook log, and surfaces error count + first line.
...
- `acd start` short-circuit: reads per-session start-cache and central registry; same `session_id` + fresh heartbeat + daemon fingerprint returns without control.lock, SQLite migration, or registry rewrite. Hot path should stay about 50ms. Adapter PPID `kill(0)` probe runs near `RegisterClient`; ESRCH logs informational warning and continues.
...
- `acd hook-stdin-extract <field> [field...]`: rejects CR/LF/NUL; empty required fields are field-not-found; required outputs flush only after every required field resolves; 1 MiB stdin truncation is distinct. Optional fields use `?`.
...
- `acd recover --auto` refuses while daemon PID is alive; creates `.git/acd/state.db.recover-<timestamp>`, retargets pending/blocked rows, resets blocked, clears replay/pause meta, removes manual pause marker.
...
- Marker constants are format-specific: TOML/YAML use leading `# acd-managed: true`; JSON detects `"_acd_managed": true` with/without space. Keep `internal/cli/hookhelper.go`, `internal/cli/setup_test.go`, templates, and AdapterE2E in sync.
