...
> **Status: SUPERSEDED**
> This Python implementation is frozen as a historical reference for the
> atomic-commit pattern. Production users should install `acd` from the
> project root instead. See `cmd/acd` and the top-level README.
> Lives at `.plan/examples/atomic-commit-worktree-daemon/` — gitignored, local
> reference only.
