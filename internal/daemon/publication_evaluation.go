...
func evaluatePublication[T any](ctx context.Context, run func(context.Context) (T, error)) (T, error) {
 ...
 if evaluation == nil { return run(ctx) }
 var zero T
 identity, err := evaluation.identity(ctx)
 if err != nil { evaluation.cancel(); return zero, err }
 type result struct { value T; err error }
 ...
 if evaluation.gate != nil { evaluation.gate.RUnlock() }
 locked := false
 lock := func() { if !locked && evaluation.gate != nil { evaluation.gate.RLock() }; locked = true }
 unlock := func() { if locked && evaluation.gate != nil { evaluation.gate.RUnlock() }; locked = false }
 defer lock() // restore the caller's operation-gate ownership on every exit
 interval := evaluation.interval
 if interval <= 0 { interval = 750 * time.Millisecond }
 delay := interval
 ...
 validate := func() error {
  ...
  if current != identity { return errPublicationEvaluationStale }
  return nil
 }
 for {
  activity := false
  ...
  case completed := <-done:
   lock()
   if err := validate(); err != nil { evaluation.cancel(); return zero, err }
   return completed.value, completed.err
  case <-ctx.Done(): return abort(ctx.Err())
  case <-evaluation.shutdown: return abort(context.Canceled)
  case <-evaluation.wake: activity = true
  case <-evaluation.files: activity = true
  case <-timer.C:
  }
  lock()
  if err := validate(); err != nil { unlock(); return abort(err) }
  if err := evaluation.protect(ctx); err != nil { unlock(); return abort(err) }
  unlock()
  if activity { delay = interval } else { delay = min(delay * 2, 2 * time.Minute) }
