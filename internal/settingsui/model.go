...
func (m Model) ActiveField() FieldDescriptor {
	...
	if len(fields) == 0 { return FieldDescriptor{} }
	if m.Focus < 0 { m.Focus = 0 }
	if m.Focus >= len(fields) { m.Focus = len(fields)-1 }
...
func (m Model) Fingerprint() string {
	...
	for key := range m.Draft { if !descriptor(key).Sensitive { keys = append(keys, key) } }
	sort.Strings(keys)
	var b strings.Builder
	for _, key := range keys { fmt.Fprintf(&b, "%s=%s\n", key, m.Draft[key]) }
	return b.String()
}

func (m Model) snapshotCmd() tea.Cmd {
	if m.backend == nil { return nil }
	return func() tea.Msg { s, err := m.backend.Snapshot(context.Background()); return snapshotMsg{Snapshot:s, err:err} }
}
